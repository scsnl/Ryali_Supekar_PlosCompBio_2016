...
init_option = 2; %1-random, 2-task,3-kmeans
TR = 0.75;
rat_ids = {'2010_10_21_cage26','2010_11_09_cage24'};
rat_ids(1) = rat_ids(1);
stim_freq = '/20Hz/';
saveDir = '/mnt/mapricot/musk2/home/sryali/Work/switchingFC/VB-HMM/Results/HCP/';
fname = 'optoFmri_task_regress_out_2ROIs_25Clusters_lambda_1000.mat';
figure
for n = 1:length(rat_ids)
    timeseries_dir = ['/mnt/mapricot/musk2/home/sryali/Work/Causal_Analysis_DCM/', ...
        'Revision/ofMRI-Stanford-Test/Data/Scan_wise/',rat_ids{n},stim_freq];
    fname_data1 = [timeseries_dir, 'avg_cx_thal', '.mat'];
    load(fname_data1)
    yres = regress_task_out(avg_ts,stim_design,TR);
    data{n} = yres';
    task{n} = stim_design + 1;
    subplot(length(rat_ids),1,n)
    plot(yres)
    hold on
    plot(task{n} + 0.01,'r','linewidth',2)
end

% VB-HMM
%matlabpool local 8
%[Model]= vb_cont_hmm_multi_subj(data,K,maxIter,tol,no_repetitions,task,init_option);
%matlabpool close
load([saveDir fname])
